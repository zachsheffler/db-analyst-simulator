...
|---|---|---|
| **1 · Design** | Reads a business brief, draws an ER diagram (entities, attributes, relationships with cardinality/participation, weak entities, composite/multivalued/derived attributes), maps it to a relational schema (tables, PKs, FKs), then writes `CREATE TABLE` statements | Each step is compared with a reference solution using lenient name matching; the DDL is executed in a real SQLite database and probed with INSERT tests that must succeed or be rejected |
...
| **3 · Present** | Builds the right visual for a business question in a Power BI-style report canvas (Fields / Visualizations / Filters panes, field wells, model view) | Chart type, fields, aggregation, filters, sort, title, and visualization principles are checked; equivalent field choices are accepted when the data matches |
...
|---|---|---|---|
| **Dave's Gig Log** 🛵 | Easy | 5 tables: his car, scooter and bicycle, four gig apps, gigs and expenses | 1 design job, 30 SQL templates, 6 viz jobs |
| **Ulysses Tech Tips** 🎬 | Medium | 9 tables: departments, employees (with managers), channels, videos, products, manufacturers, sponsors, sponsorships | 2 design jobs, 40 SQL templates (incl. self-joins), 6 viz jobs |
| **Mega EpicGames** 🎮 | Hard | 13 tables: studios, games, genres, platforms, customers, store sales with a revenue cut, reviews, hardware products/orders/lines, credit accounts and a weak-entity transaction ledger | 2 design jobs, 42 SQL templates, 8 viz jobs |
...
| **Bottom (30 %)** | Timer & game controls: stopwatch or countdown, scores, Check / Run / Submit / Skip / Pause / Leave | Chat (placeholder, to be wired up later) |
...
- `dave-challenges.json`, `utt-challenges.json`, `mega-challenges.json` — the jobs for each employer.
...
- **Query sprint**: base points by difficulty (10/15/20/30/40) × streak multiplier (+10 % per consecutive correct, max 2×) × speed bonus (1.5× under 60 s, 1.25× under 2 min). Second attempt 60 %, third+ 30 %, revealed solution 0. Skips are free but reset the streak.
- **Present**: 100 points per challenge; best score kept.
...
src/
  ...
  components/             Slots (help/controls panels), ControlBar + stopwatch, grade report, tables
  modules/design/         ER canvas + editor, schema editor + diagram, DDL step
  modules/query/          sprint UI
  modules/present/        Power BI pastiche + SVG charts
  pages/                  company picker, chat placeholder, report card, packs
...
- The presentation builder has no calculated measures; store computed values (e.g. a line total) as columns in the database.
